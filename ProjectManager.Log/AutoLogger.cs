...
namespace ProjectManager.Log
{
    public class AutoLogger
    {
        ...
        private static NLog.Logger logger = LogManager.GetLogger("autofile");
        ...
        private static string GetUserInfo()
        {
            ...
            if (user != null)
            {
                if (user is Common.LoginUserInfo.LoginUserPrincipal)
                {
                    Common.LoginUserInfo.LoginUserPrincipal info = (Common.LoginUserInfo.LoginUserPrincipal)user;
                    return string.Format("[{0:N}:{1}] - ", info.ID, info.UserName);
                }
            }
            return string.Empty;
        }
